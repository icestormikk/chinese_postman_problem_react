...
function LaunchGenetic() {
  ...
  const [status, setStatus] = React.useState<"SUCCESS"|"FAILED">()
  ...
  return (
    <form onSubmit={onGeneticAlgorithmLaunch} className='flex flex-col gap-2 p-2 rounded-md shadow-md border border-solid border-gray-200 w-full'>
      ...
      <div className='flex flex-col gap-4 justify-center items-start'>
        ...
        {
          status === "SUCCESS" && ( <SuccessMessage message={'Алгоритм завершил свою работу успешно'}/> )
        }
        {
          status === "FAILED" && ( <ErrorMessage message={'Во время работы алгоритма произошла ошибка'}/> )
        }
      </div>
    </form>
  );
